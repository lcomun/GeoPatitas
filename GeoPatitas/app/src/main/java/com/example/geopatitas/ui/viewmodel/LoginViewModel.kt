...
class LoginViewModel(
    ...
    fun handleGoogleSignInResult(task: Task<GoogleSignInAccount>) {
        viewModelScope.launch {
            ...
            try {
                ...
                _uiState.value =
                    LoginUiState.Error(e.message ?: "Fallo el inicio de sesión de Google.")
            }
        }
    }
    ...
    fun authenticateWithFirebaseAndCheckRegistration(googleAccount: GoogleSignInAccount) {
        viewModelScope.launch {
            ...
            when (val authRes = authRepository.signInWithGoogleCredentialFirebaseUser(credential)) {
                is AuthRes.Success -> {
    ...
    fun signOut() {
        viewModelScope.launch {
            ...
            when (val result = authRepository.signOutFirebaseUser()) {
                is AuthRes.Success -> {
                    ...
                    googleSignInClient.signOut().addOnCompleteListener {
                        ...
                        _uiState.value =
                            LoginUiState.Idle // Restablece el estado a "Idle" cuando se cierra sesión
                    }
                }
                is AuthRes.Error -> {
                    Log.e("LoginViewModel", "signOut: Error al cerrar sesión en Firebase. ${result.errorMessage}")
                    _uiState.value =
                        LoginUiState.Error("Error al cerrar sesión: ${result.errorMessage}")
                }
            }
        }
    }
